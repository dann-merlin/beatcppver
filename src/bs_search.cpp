...
void BsSearchResult::process_response(BsResponse response) {
	...
	for(auto json_entry : json_results) {
		...
		for(nlohmann::detail::iteration_proxy_value<nlohmann::detail::iter_impl<nlohmann::basic_json<std::map, std::vector, std::__cxx11::basic_string<char>, bool, long, unsigned long, double, std::allocator, nlohmann::adl_serializer> > >& it : difficulties.items()) {
